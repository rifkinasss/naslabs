...
export default async function CaseStudyPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return <main className="case-study section-shell"><Link className="back-link" href="/work"><ArrowLeft aria-hidden="true" /> Back to work</Link><header className="case-hero"><p className="eyebrow"><span className="eyebrow-dot" /> {project.category}</p><h1>{project.title}</h1><p>{project.summary}</p></header>{project.image && <div className={`case-image-frame case-image-frame--${project.slug}`}><Image className="case-image" src={project.image} alt={`${project.title} website hero screenshot`} fill sizes="(max-width: 900px) 100vw, 1200px" priority /></div>}<div className="case-meta"><div><span>Context</span><p>{project.context}</p></div><div><span>Role</span><p>{project.role}</p></div><div><span>Technology</span><p>{project.stack.join(" · ")}</p></div></div><div className="case-sections">{project.sections.map((section) => <section key={section.title}><span className="service-number">{section.title}</span><p>{section.body}</p></section>)}</div><section className="page-cta"><div><p className="eyebrow">Have a similar challenge?</p><h2>Let&apos;s talk<br /><em>through it.</em></h2></div><Link className="text-link" href="/contact">Start a conversation <ArrowUpRight aria-hidden="true" /></Link></section></main>;
}
